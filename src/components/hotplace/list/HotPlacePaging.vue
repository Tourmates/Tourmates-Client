        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item" v-if="prev">
                    <router-link
                            :to="`/hotPlaces?pageNumber=${ (startPageIndex - 1) * listRowCount }`"
                ...
                <li class="page-item" v-for="index in endPageIndex - startPageIndex + 1" :key="index"
                    ...
                    <router-link
                            :to="`/hotPlaces?pageNumber=${ ((startPageIndex + index - 1) * listRowCount) / 10 }`"
                            class="page-link"
                ...
                <li class="page-item" v-if="next">
                    <router-link
                            :to="`/hotPlaces?pageNumber=${ (endPageIndex + 1) * listRowCount }`"
...
export default {
    ...
    methods: {
        ...
        init() {
            const API_URL = "http://localhost:8080/hotPlaces/totalCount";
            axios.get(API_URL)
                .then((response) => {
    ...
    mounted() {
        this.$router.push('hotPlaces?pageNumber=' + (this.listRowCount / 10))
            .catch(() => {
            });
    }
};
