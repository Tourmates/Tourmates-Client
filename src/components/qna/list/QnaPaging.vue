        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item" v-if="prev">
                    <router-link
                            :to="`/qna?pageNumber=${ (startPageIndex - 1) * listRowCount }`"
                ...
                <li class="page-item" v-for="index in endPageIndex - startPageIndex + 1" :key="index"
                    ...
                    <router-link
                            :to="`/qna?pageNumber=${ (startPageIndex + index - 1) * listRowCount }`"
                            class="page-link"
                ...
                <li class="page-item" v-if="next">
                    <router-link
                            :to="`/qna?pageNumber=${ (endPageIndex + 1) * listRowCount }`"
...
export default {
    ...
    methods: {
        ...
        initComponent() {
            const API_URL = "http://localhost:8080/qna/totalCount";
            axios.get(API_URL)
                .then((response) => {
    ...
    mounted() {
        this.$router.push('qna?pageNumber=' + this.listRowCount)
            .catch(() => {});
    },
}
</script>